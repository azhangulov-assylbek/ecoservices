...
/* console */
const sensors = [
  {id:"so2", label:"SO₂, дымовая труба № 1", unit:"мг/нм³", base:172, noise:9, lim:200, dec:0, spike:true, c:"var(--c-climate)"},
  {id:"pm",  label:"PM10, граница СЗЗ", unit:"мкг/м³", base:38, noise:4, lim:60, dec:0, c:"var(--c-health)"},
  {id:"ph",  label:"pH, водоём-охладитель", unit:"", base:7.4, noise:.06, lim:8.5, dec:2, c:"var(--c-water)"},
  {id:"db",  label:"Шум, жилая зона", unit:"дБА", base:46, noise:1.5, lim:55, dec:0, c:"var(--c-health)"}
];
...
sensors.forEach(s=>{
  ...
  d.innerHTML = `<span class="t-label">${s.label}</span><span><span class="t-val num">0</span><span class="t-unit">${s.unit}</span></span>
    <span class="t-state">в норме</span><svg viewBox="0 0 100 34" preserveAspectRatio="none" aria-hidden="true"><line class="lim" x1="0" x2="100"/><polyline class="spark"/></svg>`;
...
function draw(s){
  ...
  t.querySelector(".t-state").textContent = al?`выше ориентира ${s.lim}`:"в норме";
  if(al&&!s.alarm){const pct=Math.round((v/s.lim-1)*100);addLog(`${s.label.split(",")[0]}: превышение на ${pct}%`,true);if(s.id==="so2")explain(Math.round(v),pct);}
  if(!al&&s.alarm)addLog(`${s.label.split(",")[0]}: вернулся в норму`,false);
  s.alarm=al;
}
function step(){
...
function explain(val,pct){
  const parts=[`Концентрация SO₂ ${val} мг/нм³, на ${pct}% выше ориентира НДТ для угольного котла этой мощности.`,
    `Вероятная причина: рост содержания серы в поставке угля. Рекомендуемые техники: полусухая или мокрая известняковая сероочистка, контроль качества топлива.`];
  const src=`Справочник НДТ «Сжигание топлива на крупных установках в целях производства энергии», ПП РК от 23.01.2024 № 23`;
...
sensors.forEach(draw); addLog("Подключено 4 поста мониторинга",false); step(); setInterval(step,1500);
...
document.addEventListener("click",e=>{
  ...
  b.setAttribute("aria-pressed",on); b.textContent=on?"Сообщим о запуске":"Сообщить о запуске";
});
...
if(quote){
  ...
  document.querySelectorAll("[data-quote]").forEach(b=>b.addEventListener("click",()=>{
    document.getElementById("quote-title").textContent="Запрос предложения: "+b.dataset.quoteName;
